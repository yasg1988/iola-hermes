...
Дополнительно в `apps/desktop-tauri` начата экспериментальная легкая
Tauri-оболочка на Rust. Она не заменяет Electron-релиз: это отдельная сборка
для проверки памяти, размера установщика и скорости запуска. Запуск проверки:
`npm run tauri:check`.
