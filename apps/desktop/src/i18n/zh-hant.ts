  ...
  settings: {
    ...
    model: {
      ...
      staleAuxWarning: (count, names, provider) =>
        `${count} 個輔助任務（${names}）仍在 ${provider} 上執行，而不是主要模型。`,
      auxiliaryTitle: '輔助模型',
      ...
      providerNeedsApiKey: provider => `${provider} 需要 API 金鑰，請先設定後再選擇模型。`,
      providerSignsInBrowser: provider => `${provider} 透過瀏覽器登入，Hermes 會為您執行該流程。`,
      tasks: {
