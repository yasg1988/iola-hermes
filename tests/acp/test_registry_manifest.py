...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "iola-hermes"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Hermes RU Iola"
    assert data["description"]
    assert data["repository"] == "https://github.com/yasg1988/iola-hermes"
    assert data["website"].startswith("https://github.com/yasg1988/iola-hermes")
    assert data["authors"] == ["Yakunin"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"iola-hermes[acp]=={data['version']}"
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"iola-hermes[acp]=={_pyproject_version()}"
    )
