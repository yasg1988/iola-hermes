...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"iola-hermes[termux]"' in text
    assert '"iola-hermes[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"iola-hermes[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
